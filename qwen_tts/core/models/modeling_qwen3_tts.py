...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        min_text_len = min(input_id.shape[-1] - 8 for input_id in input_ids)
        min_speech_tokens = int(min_text_len * 1.5)
        print(f"[Generate] batch_size={len(input_ids)}, min_text_len={min_text_len}, min_speech_tokens={min_speech_tokens}")

        talker_kwargs = {
            "max_new_tokens": max_new_tokens,
            "min_new_tokens": min_speech_tokens,
            "do_sample": do_sample,
            "top_k": top_k,
            "top_p": top_p,
